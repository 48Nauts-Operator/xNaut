...
fn normalize_project_path(raw: &str) -> PathBuf {
    ...
    // If the user passed a path ending in prd.json, use its parent directory
    if expanded.file_name().and_then(|f| f.to_str()) == Some("prd.json") {
...
pub async fn ralph_read_prd(project_path: String) -> Result<serde_json::Value, String> {
    let prd_path = normalize_project_path(&project_path).join("prd.json");
...
pub async fn ralph_backup_prd(project_path: String) -> Result<String, String> {
    let base = normalize_project_path(&project_path);
    let prd_path = base.join("prd.json");
    let backup_dir = base.join(".ralph-backups");
