...
class RalphOrchestrator {
  ...
  async loadProject(projectPath) {
    // Normalize: the Rust side handles ~/prd.json for ralph_read_prd,
    // but we also need a clean directory path for workingDir in command sessions.
    this.projectPath = this._normalizePath(projectPath);
    try {
      this.prd = await invoke('ralph_read_prd', { projectPath: this.projectPath });
  ...
  /**
   * Normalize a project path: strip trailing prd.json, but keep ~ since Rust normalizes it.
   * @param {string} raw
   * @returns {string}
   */
  _normalizePath(raw) {
    ...
    // Strip trailing prd.json filename
    if (p.endsWith('/prd.json') || p.endsWith('\\prd.json')) {
