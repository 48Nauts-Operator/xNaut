...
`;
  ...
  async function createTasksPanel(tabId, parentContainer, opts) {
    ...
    function renderTriage(result, error) {
      ...
      host.innerHTML = `<div class="taskp-triage-head"><strong>Ticket Triage</strong><span class="taskp-triage-status">${escapeText(String(record.status || '').replaceAll('_', ' '))}${result.reused ? ' · existing run' : ''}</span></div><div class="taskp-triage-grid"><label>Classification</label><strong>${escapeText(String(analysis.classification || '').replaceAll('_', ' '))}</strong><label>Confidence</label><span>${Math.round(Number(analysis.confidence || 0) * 100)}%</span><label>Severity</label><span>${escapeText(analysis.severity || 'unknown')}</span><label>Components</label><span>${escapeText((analysis.affected_components || []).join(', ') || 'Not identified')}</span><label>Likely cause</label><span>${escapeText(analysis.likely_cause || '')}</span><label>Next step</label><span>${escapeText(analysis.recommended_next_step || '')}</span><label>Run</label><span>${escapeText(record.run_id || '')}</span>${record.change_id ? `<label>Change</label><strong>${escapeText(record.change_id)}</strong>` : ''}${record.change_error ? `<label>Change error</label><span style="color:#f85149">${escapeText(record.change_error)}</span>` : ''}</div>${evidence ? `<ul class="taskp-triage-evidence">${evidence}</ul>` : ''}${waiting ? '<div class="taskp-triage-decisions"><button class="taskp-action taskp-triage-reject">Reject</button><button class="taskp-action taskp-action-primary taskp-triage-approve">Approve</button></div>' : ''}`;
      if (waiting) {
